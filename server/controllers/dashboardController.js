...
async function getStats(req, res) {
  try {
    ...
    const totalSignups = await Signup.countDocuments({ waitlistId: waitlist._id });
    ...
    const referralRate = totalSignups > 0 ? Math.round((referredCount / totalSignups) * 100) : 0;
    ...
    const chartDataRaw = await Signup.aggregate([
      { $match: { waitlistId: waitlist._id, createdAt: { $gte: thirtyDaysAgo } } },
      {
        $group: {
    ...
    const chartData = chartDataRaw.map((d) => ({ date: d._id, signups: d.count }));

    res.json({ waitlist, totalSignups, signupsToday, referralRate, topReferrers, chartData });
  } catch (err) {
...
async function exportCsv(req, res) {
  try {
    ...
    const signups = await Signup.find({ waitlistId: waitlist._id }).sort({ currentPosition: 1 });

    const header = "email,position,referralCount,joinedAt\n";
    const rows = signups
      .map((s) => `${s.email},${s.currentPosition},${s.referralCount},${s.createdAt.toISOString()}`)
      .join("\n");
